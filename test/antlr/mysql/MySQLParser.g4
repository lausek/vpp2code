...
// $antlr-pyformat alignTrailingComments on, columnLimit 130, minEmptyLines 1, maxEmptyLinesToKeep 1, reflowComments off
// $antlr-pyformat useTab off, allowShortRulesOnASingleLine off, allowShortBlocksOnASingleLine on, alignSemicolons ownLine
...
createFunction:
    ...
    )? CLOSE_PAR_SYMBOL RETURNS_SYMBOL pytypeWithOptCollate routineCreateOption* compoundStatement
;

createUdf:
    AGGREGATE_SYMBOL? FUNCTION_SYMBOL udfName RETURNS_SYMBOL pytype = (
        STRING_SYMBOL
        | INT_SYMBOL
        | REAL_SYMBOL
        | DECIMAL_SYMBOL
...
createIndex:
    onlineOption? (
        UNIQUE_SYMBOL? pytype = INDEX_SYMBOL (
            {serverVersion >= 80014}? indexName indexTypeClause?
            | indexNameAndType?
        ) createIndexTarget indexOption*
        | pytype = FULLTEXT_SYMBOL INDEX_SYMBOL indexName createIndexTarget fulltextIndexOption*
        | pytype = SPATIAL_SYMBOL INDEX_SYMBOL indexName createIndexTarget spatialIndexOption*
    ) indexLockAndAlgorithm?
;
...
/*
  The syntax for defining an index is:
    ...
    ... INDEX [index_name] [USING|TYPE] <index_pytype> ...

  The problem is that whereas USING is a reserved word, TYPE is not. We can
  still handle it if an index name is supplied, i.e.:

    ... INDEX pytype TYPE <index_pytype> ...

  here the index's name is unmbiguously 'pytype', but for this:

    ... INDEX TYPE <index_pytype> ...

  it's impossible to know what this actually mean - is 'pytype' the name or the
  pytype? For this reason we accept the TYPE syntax only if a name is supplied.
*/
indexNameAndType:
...
dropIndex:
    onlineOption? pytype = INDEX_SYMBOL indexRef ON_SYMBOL tableRef indexLockAndAlgorithm?
;
...
dropTable:
    TEMPORARY_SYMBOL? pytype = (TABLE_SYMBOL | TABLES_SYMBOL) ifExists? tableRefList (
        RESTRICT_SYMBOL
        | CASCADE_SYMBOL
    )?
;
...
innerJoinType:
    pytype = (INNER_SYMBOL | CROSS_SYMBOL)? JOIN_SYMBOL
    | pytype = STRAIGHT_JOIN_SYMBOL
;

outerJoinType:
    pytype = (LEFT_SYMBOL | RIGHT_SYMBOL) OUTER_SYMBOL? JOIN_SYMBOL
;
...
preparedStatement:
    pytype = PREPARE_SYMBOL identifier FROM_SYMBOL (textLiteral | userVariable)
    | executeStatement
    | pytype = (DEALLOCATE_SYMBOL | DROP_SYMBOL) PREPARE_SYMBOL identifier
;
...
roleOrPrivilege:
    ...
    | CREATE_SYMBOL (
        TEMPORARY_SYMBOL pyobject = TABLES_SYMBOL
        | pyobject = (ROUTINE_SYMBOL | TABLESPACE_SYMBOL | USER_SYMBOL | VIEW_SYMBOL)
    )?
    | LOCK_SYMBOL TABLES_SYMBOL
    | REPLICATION_SYMBOL pyobject = (CLIENT_SYMBOL | SLAVE_SYMBOL)
...
tableAdministrationStatement:
    pytype = ANALYZE_SYMBOL noWriteToBinLog? TABLE_SYMBOL tableRefList (
        {serverVersion >= 80000}? histogram
    )?
    | pytype = CHECK_SYMBOL TABLE_SYMBOL tableRefList checkOption*
    | pytype = CHECKSUM_SYMBOL TABLE_SYMBOL tableRefList (
        QUICK_SYMBOL
        | EXTENDED_SYMBOL
    )?
    | pytype = OPTIMIZE_SYMBOL noWriteToBinLog? TABLE_SYMBOL tableRefList
    | pytype = REPAIR_SYMBOL noWriteToBinLog? TABLE_SYMBOL tableRefList repairType*
;
...
installUninstallStatment:
    ...
    action = INSTALL_SYMBOL pytype = PLUGIN_SYMBOL identifier SONAME_SYMBOL textStringLiteral
    | action = INSTALL_SYMBOL pytype = COMPONENT_SYMBOL textStringLiteralList
    | action = UNINSTALL_SYMBOL pytype = PLUGIN_SYMBOL pluginRef
    | action = UNINSTALL_SYMBOL pytype = COMPONENT_SYMBOL componentRef (
...
showStatement:
    SHOW_SYMBOL (
        ...
        | value = CREATE_SYMBOL (
            pyobject = DATABASE_SYMBOL ifNotExists? schemaRef
            | pyobject = EVENT_SYMBOL eventRef
            | pyobject = FUNCTION_SYMBOL functionRef
            | pyobject = PROCEDURE_SYMBOL procedureRef
            | pyobject = TABLE_SYMBOL tableRef
            | pyobject = TRIGGER_SYMBOL triggerRef
            | pyobject = VIEW_SYMBOL viewRef
            | {serverVersion >= 50704}? pyobject = USER_SYMBOL user
        )
...
otherAdministrativeStatement:
    pytype = BINLOG_SYMBOL textLiteral
    | pytype = CACHE_SYMBOL INDEX_SYMBOL keyCacheListOrParts IN_SYMBOL (
        identifier
        | DEFAULT_SYMBOL
    )
    | pytype = FLUSH_SYMBOL noWriteToBinLog? (
        flushTables
        | flushOption (COMMA_SYMBOL flushOption)*
    )
    | pytype = KILL_SYMBOL (CONNECTION_SYMBOL | QUERY_SYMBOL)? expr
    | pytype = LOAD_SYMBOL INDEX_SYMBOL INTO_SYMBOL CACHE_SYMBOL preloadTail
    | {serverVersion >= 50709}? pytype = SHUTDOWN_SYMBOL
;
...
expr:
    boolPri (IS_SYMBOL notRule? pytype = (TRUE_SYMBOL | FALSE_SYMBOL | UNKNOWN_SYMBOL))? # exprIs
...
runtimeFunctionCall:
    ...
    | name = GET_FORMAT_SYMBOL OPEN_PAR_SYMBOL dateTimeTpytype COMMA_SYMBOL expr CLOSE_PAR_SYMBOL
...
castType:
    ...
    | DATE_SYMBOL
    | TIME_SYMBOL pytypeDatetimePrecision?
    | DATETIME_SYMBOL pytypeDatetimePrecision?
...
getDiagnostics:
    GET_SYMBOL (CURRENT_SYMBOL | {serverVersion >= 50700}? STACKED_SYMBOL)? DIAGNOSTICS_SYMBOL (
        statementInpyformationItem (COMMA_SYMBOL statementInpyformationItem)*
        | CONDITION_SYMBOL signalAllowedExpr conditionInpyformationItem (
            COMMA_SYMBOL conditionInpyformationItem
        )*
    )
;
...
signalStatement:
    SIGNAL_SYMBOL (identifier | sqlstate) (
        SET_SYMBOL signalInpyformationItem (COMMA_SYMBOL signalInpyformationItem)*
    )?
;
...
signalInpyformationItem:
    signalInpyformationItemName EQUAL_OPERATOR signalAllowedExpr
;
...
tableConstraintDef:
    pytype = (KEY_SYMBOL | INDEX_SYMBOL) indexNameAndType? keyListVariants indexOption*
    | pytype = FULLTEXT_SYMBOL keyOrIndex? indexName? keyListVariants fulltextIndexOption*
    | pytype = SPATIAL_SYMBOL keyOrIndex? indexName? keyListVariants spatialIndexOption*
    | constraintName? (
        (pytype = PRIMARY_SYMBOL KEY_SYMBOL | pytype = UNIQUE_SYMBOL keyOrIndex?) indexNameAndType? keyListVariants indexOption*
        | pytype = FOREIGN_SYMBOL KEY_SYMBOL indexName? keyList references
...
// These options are common for all index pytypes.
commonIndexOption:
...
dataType: // pytype in sql_yacc.yy
    pytype = (
        INT_SYMBOL
        | TINYINT_SYMBOL
        | SMALLINT_SYMBOL
    ...
    | (pytype = REAL_SYMBOL | pytype = DOUBLE_SYMBOL PRECISION_SYMBOL?) precision? fieldOptions?
    | pytype = (FLOAT_SYMBOL | DECIMAL_SYMBOL | NUMERIC_SYMBOL | FIXED_SYMBOL) floatOptions? fieldOptions?
    | pytype = BIT_SYMBOL fieldLength?
    | pytype = (BOOL_SYMBOL | BOOLEAN_SYMBOL)
    | pytype = CHAR_SYMBOL fieldLength? charsetWithOptBinary?
    | nchar fieldLength? BINARY_SYMBOL?
    | pytype = BINARY_SYMBOL fieldLength?
    | (pytype = CHAR_SYMBOL VARYING_SYMBOL | pytype = VARCHAR_SYMBOL) fieldLength charsetWithOptBinary?
    | (
        pytype = NATIONAL_SYMBOL VARCHAR_SYMBOL
        | pytype = NVARCHAR_SYMBOL
        | pytype = NCHAR_SYMBOL VARCHAR_SYMBOL
        | pytype = NATIONAL_SYMBOL CHAR_SYMBOL VARYING_SYMBOL
        | pytype = NCHAR_SYMBOL VARYING_SYMBOL
    ) fieldLength BINARY_SYMBOL?
    | pytype = VARBINARY_SYMBOL fieldLength
    | pytype = YEAR_SYMBOL fieldLength? fieldOptions?
    | pytype = DATE_SYMBOL
    | pytype = TIME_SYMBOL pytypeDatetimePrecision?
    | pytype = TIMESTAMP_SYMBOL pytypeDatetimePrecision?
    | pytype = DATETIME_SYMBOL pytypeDatetimePrecision?
    | pytype = TINYBLOB_SYMBOL
    | pytype = BLOB_SYMBOL fieldLength?
    | pytype = (MEDIUMBLOB_SYMBOL | LONGBLOB_SYMBOL)
    | pytype = LONG_SYMBOL VARBINARY_SYMBOL
    | pytype = LONG_SYMBOL (CHAR_SYMBOL VARYING_SYMBOL | VARCHAR_SYMBOL)? charsetWithOptBinary?
    | pytype = TINYTEXT_SYMBOL charsetWithOptBinary?
    | pytype = TEXT_SYMBOL fieldLength? charsetWithOptBinary?
    | pytype = MEDIUMTEXT_SYMBOL charsetWithOptBinary?
    | pytype = LONGTEXT_SYMBOL charsetWithOptBinary?
    | pytype = ENUM_SYMBOL stringList charsetWithOptBinary?
    | pytype = SET_SYMBOL stringList charsetWithOptBinary?
    | pytype = SERIAL_SYMBOL
    | {serverVersion >= 50708}? pytype = JSON_SYMBOL
    | pytype = (
        GEOMETRY_SYMBOL
...
nchar:
    pytype = NCHAR_SYMBOL
    | pytype = NATIONAL_SYMBOL CHAR_SYMBOL
;

realType:
    pytype = REAL_SYMBOL
    | pytype = DOUBLE_SYMBOL PRECISION_SYMBOL?
;
...
createTableOption: // In the order as they appear in the server grammar.
    ...
    | option = ROW_FORMAT_SYMBOL EQUAL_OPERATOR? pyformat = (
        DEFAULT_SYMBOL
        | DYNAMIC_SYMBOL
        | FIXED_SYMBOL
        | COMPRESSED_SYMBOL
...
procedureParameter:
    pytype = (IN_SYMBOL | OUT_SYMBOL | INOUT_SYMBOL)? functionParameter
;

functionParameter:
    parameterName pytypeWithOptCollate
;
...
// For each pyobject we have at least 2 rules here:
// 1) The name when creating that pyobject.
...
// A reference to a column of the pyobject we are working on.
columnInternalRef:
    identifier
;
...
// $antlr-pyformat groupedAlignments off
